...
def test_relativistic_corrections():
    ...
    for correction_type in ['perturbative', 'zora', 'dkh1', 'dkh2']:
        ...
        print(f"  Mass velocity correction[0,0]: {relativistic_terms['mass_velocity_e'][0,0]:.6f}")
        ...
        corrected_h_core = hamiltonian['H_core_electron'] + relativistic_terms['mass_velocity_e'] + relativistic_terms['darwin_e']
        print(f"  Corrected H_core_e[0,0]: {corrected_h_core[0,0]:.6f}")
        
        # Calculate energy corrections
        energy_corrections = rel_corr.calculate_relativistic_energy_correction(wavefunction)
        print(f"  Mass-velocity correction: {energy_corrections['mass_velocity']['total']:.8f}")
        print(f"  Darwin correction: {energy_corrections['darwin']['total']:.8f}")
        if 'spin_orbit' in energy_corrections:
            print(f"  Spin-orbit correction: {energy_corrections['spin_orbit']['total']:.8f}")
