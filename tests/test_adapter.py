...
from antinature.qiskit_integration.adapter import QiskitNatureAdapter, PositroniumAdapter
...
def test_qiskit_nature_adapter():
    ...
    for mapper_type in ['jordan_wigner', 'parity']:
        ...
        try:
            ...
            print(f"    Successfully created PositroniumAdapter with {mapper_type} mapper")
            ...
            print(f"    Error creating PositroniumAdapter with {mapper_type} mapper: {str(e)}")
...
def test_positronium_adapter():
    ...
    try:
        ...
        if hasattr(adapter, 'create_ortho_para_hamiltonian'):
            ...
            print("    Method 'create_ortho_para_hamiltonian' not available in this version")
    except Exception as e:
    ...
    try:
        if hasattr(adapter, 'create_varying_attraction_hamiltonians'):
            ...
            print(f"    Created {len(hamiltonians)} Hamiltonians with different attractions")
            
            for attraction, op in hamiltonians.items():
                print(f"      Attraction = {attraction}: {len(op)} terms, {op.num_qubits} qubits")
        else:
            print("    Method 'create_varying_attraction_hamiltonians' not available in this version")
            
            # Alternative: Create multiple Hamiltonians directly
            attractions = [-0.5, -1.0, -1.5]
            print(f"    Creating {len(attractions)} Hamiltonians with different attractions directly:")
