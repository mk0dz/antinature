...
def test_vqe_solver():
    ...
    for optimizer in optimizers_to_test:
        ...
        try:
            ...
            print(f"  Execution time: {results.get('execution_time', results.get('optimizer_time', 'N/A'))} seconds")
            ...
            try:
                ...
                if 'quality_metrics' in analysis:
                    print(f"    Quality rating: {analysis['quality_metrics'].get('quality_rating', 'N/A')}")
            ...
            try:
                ...
                if hasattr(vqe, 'plot_convergence'):
                    fig = vqe.plot_convergence(results, save_path=f"convergence_{optimizer}.png")
    ...
    try:
        if hasattr(vqe, 'compare_optimizers'):
            ...
            for opt, result in comparison.items():
                if 'energy' in result:
                    print(f"    {opt}: Energy = {result['energy']:.6f}, Time = {result['time']:.2f}s")
                else:
