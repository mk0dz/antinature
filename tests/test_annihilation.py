...
def test_annihilation_operator():
    ...
    try:
        fig = ann_op.plot_annihilation_density(plot_type='contour', save_path='annihilation_density.png')
        ...
        try:
            ...
            print(f"  Visualization data available with keys: {list(dens_data.keys()) if isinstance(dens_data, dict) else 'N/A'}")
        except (AttributeError, TypeError) as e:
