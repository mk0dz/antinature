...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "requires_qiskit_nature: mark test as requiring qiskit_nature"
    )
    config.addinivalue_line(
        "markers",
        "requires_qiskit_algorithms: mark test as requiring qiskit_algorithms",
    )
    config.addinivalue_line(
        "markers", "requires_qiskit_aer: mark test as requiring qiskit_aer"
    )
    config.addinivalue_line(
        "markers", "skip_on_py38: mark test to be skipped on Python 3.8"
    )
...
def pytest_runtest_setup(item):
    ...
    if (
        any(mark.name == 'requires_qiskit' for mark in item.iter_markers())
        and not HAS_QISKIT
    ):
        pytest.skip("Test requires Qiskit")

    if (
        any(mark.name == 'requires_qiskit_nature' for mark in item.iter_markers())
        and not HAS_QISKIT_NATURE
    ):
        pytest.skip("Test requires Qiskit Nature")

    if (
        any(mark.name == 'requires_qiskit_algorithms' for mark in item.iter_markers())
        and not HAS_QISKIT_ALGORITHMS
    ):
        pytest.skip("Test requires Qiskit Algorithms")

    if (
        any(mark.name == 'requires_qiskit_aer' for mark in item.iter_markers())
        and not HAS_QISKIT_AER
    ):
