# quantimatter/core/molecular_data.py
...
class MolecularData:
    """
    A comprehensive data structure for quantimatter molecular systems.
    ...
    def __init__(self, 
        ...
        """
        Initialize molecular data for an quantimatter system.
        ...
        self._is_quantimatter_system = (n_positrons > 0)
    ...
    def _get_atomic_mass(element: str) -> float:
        ...
        atomic_masses = {
            'H': 1.00782503, 'He': 4.00260325, 'Li': 7.0160045, 'Be': 9.0121822,
            'B': 11.0093055, 'C': 12.0000000, 'N': 14.0030740, 'O': 15.9949146,
            'F': 18.9984032, 'Ne': 19.9924356, 'Na': 22.9897677, 'Mg': 23.9850419,
            'Al': 26.9815386, 'Si': 27.9769271, 'P': 30.9737620, 'S': 31.9720707,
            'Cl': 34.9688527, 'Ar': 39.9623831, 'K': 38.9637074, 'Ca': 39.9625906,
            'Sc': 44.9559102, 'Ti': 47.9479471, 'V': 50.9439617, 'Cr': 51.9405119,
            'Mn': 54.9380496, 'Fe': 55.9349421, 'Co': 58.9332002, 'Ni': 57.9353479,
            'Cu': 62.9296011, 'Zn': 63.9291466, 'Ga': 68.9255776, 'Ge': 73.9211782,
            'As': 74.9215964, 'Se': 79.9165218, 'Br': 78.9183376, 'Kr': 83.9115083
        }
    ...
    def get_bonds(self, scale_factor: float = 1.3) -> List[Tuple[int, int]]:
        ...
        covalent_radii = {
            'H': 0.74, 'He': 0.82, 'Li': 1.33, 'Be': 1.02, 'B': 0.85, 'C': 0.75,
    ...
    def get_formula(self) -> str:
        ...
        if self.charge != 0:
            formula += f" {'+'*self.charge if self.charge > 0 else '-'*abs(self.charge)}"
    ...
    def visualize(self, show_bonds: bool = True, save_path: str = None):
        ...
        colors = {
            'H': 'white', 'He': 'cyan', 'Li': 'purple', 'Be': 'darkgreen',
            'B': 'salmon', 'C': 'black', 'N': 'blue', 'O': 'red',
            'F': 'green', 'Ne': 'cyan', 'Na': 'purple', 'Mg': 'darkgreen',
            'Al': 'gray', 'Si': 'gold', 'P': 'orange', 'S': 'yellow',
            'Cl': 'green', 'Ar': 'cyan', 'K': 'purple', 'Ca': 'darkgreen'
        }
        
        # Element sizes based on atomic radii (scaled)
        sizes = {
            'H': 25, 'He': 35, 'Li': 145, 'Be': 105, 'B': 85, 'C': 70,
            'N': 65, 'O': 60, 'F': 50, 'Ne': 45, 'Na': 180, 'Mg': 150,
            'Al': 125, 'Si': 110, 'P': 100, 'S': 100, 'Cl': 100, 'Ar': 95,
            'K': 220, 'Ca': 180
        }
        ...
        for i, (element, position) in enumerate(self.atoms):
            ...
            ax.scatter(
                ...
                color=color, s=size, edgecolor='black', alpha=0.8,
                label=f"{i+1}: {element}" if i < 10 else ""  # Show labels for first 10 atoms
            )
            ...
            if self.n_positrons > 0 and i == 0:  # Just a visual indicator
                ax.scatter(
                    ...
                    color='magenta', s=50, edgecolor='black', alpha=0.7,
        ...
        if self._is_quantimatter_system:
            title += " (quantimatter System)"
        ax.set_title(title)
    ...
    @classmethod
    def from_xyz(cls, filename: str, n_electrons: int = None, n_positrons: int = 0, charge: int = 0):
        """
        ...
        if n_electrons is None:
            n_electrons = sum(cls._get_atomic_number(atom[0]) for atom in atoms) - charge
