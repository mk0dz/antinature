...
class AntinatureCorrelation:
    ...
    def __init__(self, 
        ...
        if self.frozen_core:
            ...
            self.n_e_core = min(1, self.n_e_occ)  # At least keep 1s orbital frozen if possible
    ...
    def transform_eri_to_mo_basis(self, 
                                 eri_ao: Union[np.ndarray, Any], 
                                 C: np.ndarray, 
                                 optimization_level: int = 2) -> np.ndarray:
        """
    ...
    def transform_ep_attraction_to_mo_basis(self, 
                                          eri_ao: Union[np.ndarray, Any],
                                          C_e: np.ndarray, 
                                          C_p: np.ndarray,
                                          optimization_level: int = 2) -> np.ndarray:
                                    ...
                                    for ν in range(n_e_ao):
                                        for λ in range(n_p_ao):
                                            for σ in range(n_p_ao):
                                                eri_mo[p, q, r, s] += (
                                                    C_e[μ, p] * C_e[ν, q] * C_p[λ, r] * C_p[σ, s] * 
                                                    eri_ao[μ, ν, λ, σ]
                                                )
            else:
                # High optimization - use tensor operations
    ...
    def mp2_energy(self, include_electron_positron: bool = True):
        ...
        if self.C_electron is not None and self.electron_repulsion is not None and self.n_e_occ > 0 and self.n_e_virt > 0:
                ...
                for j in range(self.n_e_core, self.n_e_occ):
                    for a in range(self.n_e_occ, self.n_e_orbitals):
                        for b in range(self.n_e_occ, self.n_e_orbitals):
                            ...
                            numerator = eri_mo_e[i, a, j, b] * (2.0 * eri_mo_e[i, a, j, b] - eri_mo_e[i, b, j, a])
                            ...
                            if abs(denominator) < 1e-10:
                                if self.print_level > 0:
                                    print(f"Warning: Small denominator in MP2 (e-e): {denominator}")
                                continue
        ...
        if self.C_positron is not None and self.positron_repulsion is not None and self.n_p_occ > 0 and self.n_p_virt > 0:
            # Transform ERIs to MO basis
                ...
                for j in range(self.n_p_core, self.n_p_occ):
                    for a in range(self.n_p_occ, self.n_p_orbitals):
                        for b in range(self.n_p_occ, self.n_p_orbitals):
                            ...
                            numerator = eri_mo_p[i, a, j, b] * (2.0 * eri_mo_p[i, a, j, b] - eri_mo_p[i, b, j, a])
                            ...
                            if abs(denominator) < 1e-10:
                                if self.print_level > 0:
                                    print(f"Warning: Small denominator in MP2 (p-p): {denominator}")
                                continue
        ...
        if (include_electron_positron and 
            self.C_electron is not None and 
            self.C_positron is not None and 
            self.electron_positron_attraction is not None and
            self.n_e_occ > 0 and self.n_e_virt > 0 and
            self.n_p_occ > 0 and self.n_p_virt > 0):
            
            # Transform electron-positron attraction integrals to MO basis
            eri_mo_ep = self.transform_ep_attraction_to_mo_basis(
                self.electron_positron_attraction, 
                self.C_electron, 
                self.C_positron
            )
                ...
                for a in range(self.n_e_occ, self.n_e_orbitals):
                    for j in range(self.n_p_core, self.n_p_occ):
                        for b in range(self.n_p_occ, self.n_p_orbitals):
                            ...
                            denominator = (self.E_electron[i] - self.E_electron[a] + 
                                           self.E_positron[j] - self.E_positron[b])
                            
                            # Check for small denominator
                            if abs(denominator) < 1e-10:
                                if self.print_level > 0:
                                    print(f"Warning: Small denominator in MP2 (e-p): {denominator}")
                                continue
    ...
    def mp3_energy(self, include_electron_positron: bool = True):
        ...
        if self.C_electron is not None and self.electron_repulsion is not None and self.n_e_occ > 0 and self.n_e_virt > 0:
            ...
            if ('transform_eri', id(self.electron_repulsion), id(self.C_electron), 2) not in self._cache:
                eri_mo_e = self.transform_eri_to_mo_basis(self.electron_repulsion, self.C_electron)
            else:
                eri_mo_e = self._cache[('transform_eri', id(self.electron_repulsion), id(self.C_electron), 2)]
                        ...
                        for a in range(self.n_e_occ, self.n_e_orbitals):
                            for b in range(self.n_e_occ, self.n_e_orbitals):
                                for c in range(self.n_e_occ, self.n_e_orbitals):
                                    ...
                                    term1 = eri_mo_e[i, j, a, b] * eri_mo_e[a, b, c, k] * eri_mo_e[c, k, i, j]
                                    
                                    denom1 = (self.E_electron[i] + self.E_electron[j] - 
                                              self.E_electron[a] - self.E_electron[b])
                                    denom2 = (self.E_electron[a] + self.E_electron[b] - 
                                              self.E_electron[c] - self.E_electron[k])
        ...
        if self.C_positron is not None and self.positron_repulsion is not None and self.n_p_occ > 0 and self.n_p_virt > 0:
            # Transform ERIs to MO basis if not already cached
            if ('transform_eri', id(self.positron_repulsion), id(self.C_positron), 2) not in self._cache:
                eri_mo_p = self.transform_eri_to_mo_basis(self.positron_repulsion, self.C_positron)
            else:
                eri_mo_p = self._cache[('transform_eri', id(self.positron_repulsion), id(self.C_positron), 2)]
        ...
        if (include_electron_positron and 
            self.C_electron is not None and 
            self.C_positron is not None and 
            self.electron_positron_attraction is not None):
            
            # Transform electron-positron attraction integrals if not already cached
            if ('transform_ep', id(self.electron_positron_attraction), id(self.C_electron), id(self.C_positron), 2) not in self._cache:
                eri_mo_ep = self.transform_ep_attraction_to_mo_basis(
                    self.electron_positron_attraction, 
                    self.C_electron, 
                    self.C_positron
                ...
                eri_mo_ep = self._cache[('transform_ep', id(self.electron_positron_attraction), id(self.C_electron), id(self.C_positron), 2)]
                ...
                for j in range(self.n_e_occ):
                    for a in range(self.n_e_virt):
                        for b in range(self.n_e_virt):
                            ...
                            denominator = (self.E_electron[i] + self.E_electron[j] - 
                                           self.E_electron[a_idx] - self.E_electron[b_idx])
    ...
    def calculate_annihilation_rate(self):
        ...
        if (self.annihilation is None or self.C_electron is None or 
            ...
            if self.print_level > 0:
                print("Warning: Missing required data for annihilation rate calculation")
            return 0.0
    ...
    def calculate_correlation_energy(self, 
        ...
        if method == 'mp2':
            ...
            energy = self.ccsd_energy(include_electron_positron=include_electron_positron)
    ...
    def get_performance_report(self):
        ...
        percentages = {k: 100.0 * v / total if total > 0 else 0.0 
