...
class AntinatureIntegralEngine:
    ...
    def _hermite_overlap(self, l1, l2, PA, PB, gamma):
        ...
        if l1 == 0 and l2 == 0:
            ...
            return (
                ...
                (l2-1) * 0.5 * gamma**(-1) * self._hermite_overlap(l1, l2-2, PA, PB, gamma)
            ) if l2 > 1 else PB
        elif l2 == 0:
            # Recursion for l1 > 0
            return (
                PA * self._hermite_overlap(l1-1, l2, PA, PB, gamma) +
                (l1-1) * 0.5 * gamma**(-1) * self._hermite_overlap(l1-2, l2, PA, PB, gamma)
            ) if l1 > 1 else PA
        else:
            ...
            term3 = 0.0
            if l1 > 1:
                term3 += (l1-1) * 0.5 * gamma**(-1) * self._hermite_overlap(l1-2, l2, PA, PB, gamma)
            if l2 > 1:
                term3 += (l2-1) * 0.5 * gamma**(-1) * self._hermite_overlap(l1, l2-2, PA, PB, gamma)
    ...
    def kinetic_integral(self, basis_i, basis_j):
        ...
        if la == ma == na == lb == mb == nb == 0:
            ...
            if lb > 0:
                ...
                term_x = beta * (2*lb + 1) * self.overlap_integral(basis_i, basis_j_x_minus)
                result += term_x
            
            # Contributions from y and z
            if mb > 0:
                basis_j_y_minus = self._modify_angular_momentum(basis_j, 1, -1)
                term_y = beta * (2*mb + 1) * self.overlap_integral(basis_i, basis_j_y_minus)
                result += term_y
                
            if nb > 0:
                basis_j_z_minus = self._modify_angular_momentum(basis_j, 2, -1)
                term_z = beta * (2*nb + 1) * self.overlap_integral(basis_i, basis_j_z_minus)
                result += term_z
            
            # Term 2: -2β²<φa|φ_lb-2>
            if lb > 1:
                basis_j_x_minus2 = self._modify_angular_momentum(basis_j, 0, -2)
                term_x2 = -2 * beta**2 * self.overlap_integral(basis_i, basis_j_x_minus2)
                result += term_x2
                
            if mb > 1:
                basis_j_y_minus2 = self._modify_angular_momentum(basis_j, 1, -2)
                term_y2 = -2 * beta**2 * self.overlap_integral(basis_i, basis_j_y_minus2)
                result += term_y2
                
            if nb > 1:
                basis_j_z_minus2 = self._modify_angular_momentum(basis_j, 2, -2)
                term_z2 = -2 * beta**2 * self.overlap_integral(basis_i, basis_j_z_minus2)
                result += term_z2
    ...
    def electron_repulsion_integral(self, basis_i, basis_j, basis_k, basis_l):
        ...
        if (basis_i.angular_momentum == (0,0,0) and 
            basis_j.angular_momentum == (0,0,0) and
            basis_k.angular_momentum == (0,0,0) and
            basis_l.angular_momentum == (0,0,0)):
            ...
            # Final calculation
            result = prefactor * F0 * basis_i.normalization * basis_j.normalization * basis_k.normalization * basis_l.normalization
        else:
            ...
            L_sum = (sum(basis_i.angular_momentum) + sum(basis_j.angular_momentum) + 
                     sum(basis_k.angular_momentum) + sum(basis_l.angular_momentum))
    ...
    def electron_positron_attraction_integral(self, e_basis_i, e_basis_j, p_basis_k, p_basis_l):
        ...
        return -self.electron_repulsion_integral(e_basis_i, e_basis_j, p_basis_k, p_basis_l)
