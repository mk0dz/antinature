...
class AntinatureQuantumSystems:
    ...
    def anti_helium(self) -> Tuple[Any, QuantumCircuit]:
        ...
        for p_orb in p1_orbs + p2_orbs:
            for n_orb in nucleus:
                two_body[p_orb, p_orb, n_orb, n_orb] = -2.0  # Strong attraction (2 antiprotons)
    ...
    def custom_antinature_system(self, 
                               one_body: np.ndarray, 
        ...
        if one_body.shape != (n_orbitals, n_orbitals):
            raise ValueError(f"one_body shape expected {(n_orbitals, n_orbitals)}, got {one_body.shape}")
        
        if two_body.shape != (n_orbitals, n_orbitals, n_orbitals, n_orbitals):
            raise ValueError(f"two_body shape expected {(n_orbitals, n_orbitals, n_orbitals, n_orbitals)}, got {two_body.shape}")
