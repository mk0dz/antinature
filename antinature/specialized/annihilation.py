...
class AnnihilationOperator:
    ...
    def __init__(self, 
        ...
        if wavefunction and 'n_electrons' in wavefunction and 'n_positrons' in wavefunction:
            if wavefunction.get('n_electrons', 0) == 1 and wavefunction.get('n_positrons', 0) == 1:
                self.is_positronium = True
                print("Detected positronium system - using specialized calculation methods")
    
    def build_annihilation_operator(self, 
                                   use_vectorized: bool = True, 
                                   grid_resolution: int = 50,
                                   r_cutoff: float = 8.0) -> np.ndarray:
        """
        ...
        if self.calculation_method == 'delta':
            ...
            if use_vectorized:
                ...
                matrix = self._build_standard_operator_vectorized(grid_resolution, r_cutoff)
            else:
        ...
        if np.all(np.abs(matrix) < 1e-10):
            print("Warning: Annihilation matrix is near zero. Using approximate values.")
    ...
    def _build_standard_operator_analytical(self) -> np.ndarray:
        ...
        for i in range(n_e_basis):
            for j in range(n_p_basis):
                ...
                if (all(m == 0 for m in e_func.angular_momentum) and 
                    all(m == 0 for m in p_func.angular_momentum)):
                    ...
                    overlap = prefactor * exponential * e_func.normalization * p_func.normalization
    ...
    def _build_standard_operator_vectorized(self, 
                                           grid_resolution: int, 
                                           r_cutoff: float) -> np.ndarray:
        """
        ...
        for i, e_func in enumerate(self.basis_set.electron_basis.basis_functions):
            ...
            if all(m == 0 for m in e_func.angular_momentum):
                # Simple Gaussian
                e_values[i] = e_func.normalization * np.exp(-e_func.exponent * r_squared)
            else:
        ...
        for i, p_func in enumerate(self.basis_set.positron_basis.basis_functions):
            ...
            if all(m == 0 for m in p_func.angular_momentum):
                # Simple Gaussian
                p_values[i] = p_func.normalization * np.exp(-p_func.exponent * r_squared)
            else:
                # For other angular momentum, evaluate individually
                for j, point in enumerate(points):
    ...
    def _build_advanced_operator(self, 
                                grid_resolution: int, 
                                r_cutoff: float) -> np.ndarray:
        """
        ...
        base_matrix = self._build_standard_operator_vectorized(grid_resolution, r_cutoff)
        ...
        if self.is_positronium:
            ...
            print(f"Applied enhancement factor of {enhancement_factor:.6f} for positronium")
    ...
    def calculate_annihilation_rate(self, 
                                   ...
                                   positron_density: Optional[np.ndarray] = None) -> Dict:
        """
        ...
        lifetime = self.calculate_lifetime(total_rate) if total_rate > 0 else {'lifetime_ns': float('inf')}
    ...
    def _calculate_positronium_rates(self, 
                                    P_e: Optional[np.ndarray], 
                                    P_p: Optional[np.ndarray]) -> Dict:
        """
        ...
        if P_e is not None and P_p is not None and self.matrix is not None:
            ...
            if abs(calc_rate - theoretical_2gamma) / theoretical_2gamma < 0.5:
                ...
                print("Using theoretical positronium annihilation rate (calculated value too far off)")
        ...
        self.electron_density_at_positron = overlap if P_e is not None and P_p is not None else None
    ...
    def analyze_annihilation_channels(self, wavefunction: Optional[Dict] = None) -> Dict:
        """
        ...
        for i in range(min(n_e_occ, C_e.shape[1])):
            for j in range(min(n_p_occ, C_p.shape[1])):
                ...
                rate_2gamma += singlet_fraction * self.pi_r0_squared_c * self.matrix_mo[i, j]**2
        ...
        for i in range(min(n_e_occ, C_e.shape[1])):
            for j in range(min(n_p_occ, C_p.shape[1])):
                ...
                rate_3gamma += triplet_fraction * self.pi_r0_squared_c * triplet_to_singlet_ratio * self.matrix_mo[i, j]**2 * 3.0
        ...
        if self.is_positronium:
            ...
            theoretical_3gamma = theoretical_2gamma * triplet_to_singlet_ratio * 3.0  # Ortho-positronium 3γ rate
            ...
            for j in range(min(n_p_occ, C_p.shape[1])):
                ...
                if contribution > 1e-12:  # Only include significant contributions
                    orbital_contributions.append({
                        ...
                        'fraction': contribution / total_rate if total_rate > 0 else 0.0
                    })
    ...
    def _calculate_positronium_lifetime(self, annihilation_rate: float) -> Dict:
        ...
        theoretical_lifetime_ns = (singlet_fraction * singlet_lifetime_ns + 
                                 triplet_fraction * triplet_lifetime_ns)
        
        # Weighted uncertainty
        theoretical_uncertainty_ns = (singlet_fraction * singlet_uncertainty_ns + 
    ...
    def visualize_annihilation_density(self, 
                                      grid_dims: Tuple[int, int, int] = (50, 50, 50), 
                                      limits: Tuple[float, float] = (-5.0, 5.0),
                                      isosurface_level: Optional[float] = None) -> Dict:
        """
    ...
    def _calculate_density_at_point(self, r: np.ndarray, P: np.ndarray, basis) -> float:
        ...
        basis_vals = np.array([self._evaluate_basis_function(func, r) 
    ...
    def plot_annihilation_density(self, 
        ...
        if plot_type == '3d-isosurface':
            ...
            try:
                ...
                mesh = ax.plot_trisurf(
                    ...
                    cmap=cm.viridis, lw=0, alpha=0.7
                )
            ...
            if max_val > 0:
                levels = np.logspace(
                    np.log10(max(max_val * 0.01, 1e-10)),
                    np.log10(max_val),
                    20
                )
                
                contour = ax.contourf(
                    X_grid, Y_grid, slice_data.T,  # Transpose for correct orientation
                    levels=levels,
                    cmap=cm.plasma,
                ...
                ax.text(0.5, 0.5, 'No significant density to display',
