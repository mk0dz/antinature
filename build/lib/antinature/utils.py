...
def calculate_annihilation_rate(
    ...
    if electron_density is None or positron_density is None or overlap_matrix is None:
        warnings.warn("Missing density matrices or overlap matrix, returning zero annihilation rate")
        return 0.0
    ...
    if overlap_matrix.shape != (n_e_basis, n_p_basis):
        if overlap_matrix.shape == (n_e_basis + n_p_basis, n_e_basis + n_p_basis):
            ...
            overlap_subset = overlap_matrix[:n_e_basis, n_e_basis:n_e_basis+n_p_basis]
        else:
            warnings.warn(f"Overlap matrix has incorrect shape: {overlap_matrix.shape}, expected: {(n_e_basis, n_p_basis)}")
            overlap_subset = np.ones((n_e_basis, n_p_basis)) * 0.5  # Default approximation
    else:
    ...
    overlap_integral = np.trace(np.dot(np.dot(electron_density, overlap_subset), 
...
def calculate_density_grid(
    ...
    for i in range(density_matrix.shape[0]):
        for j in range(density_matrix.shape[1]):
            ...
            density_contribution = density_matrix[i, j] * basis_values[:, i] * basis_values[:, j]
...
def analyze_resonances(
    ...
    for i in range(1, len(energies) - 1):
        if (norm_cs[i] > norm_cs[i-1] and 
            norm_cs[i] > norm_cs[i+1] and 
            norm_cs[i] > threshold):
...
def analyze_quantum_results(
    ...
    # Only relevant for systems with both electrons and positrons
    if molecular_data.n_electrons > 0 and molecular_data.n_positrons > 0:
        annihilation_probability = 0.01 * molecular_data.n_electrons * molecular_data.n_positrons
    else:
        annihilation_probability = 0.0
...
def calculate_penetration_factor(
    ...
    # Check inputs
    if electron_density is None or positron_density is None or overlap_matrix is None:
        warnings.warn("Missing density matrices or overlap matrix, returning default penetration factor")
        return 1.0
...
def calculate_overlap_integral(
    ...
    # Check inputs
    if electron_density is None or positron_density is None or overlap_matrix is None:
        warnings.warn("Missing density matrices or overlap matrix, returning zero overlap")
        return 0.0
...
def calculate_bond_order(
    ...
    if density_matrix is None or overlap_matrix is None:
        warnings.warn("Missing density matrix or overlap matrix, returning zero bond order")
        return 0.0
