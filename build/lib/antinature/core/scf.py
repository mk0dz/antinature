...
class AntinatureSCF:
    ...
    def __init__(self, 
        ...
        self.V_nuc = molecular_data.get_nuclear_repulsion_energy() if hasattr(molecular_data, 'get_nuclear_repulsion_energy') else 0.0
    ...
    def build_fock_matrix_e(self):
        ...
        if (self.ERI_ep is not None and self.P_p is not None and 
            self.n_positrons > 0 and self.n_electrons > 0):
                    ...
                    for nu in range(n_e_basis):
                        for lambda_ in range(n_p_basis):
                            for sigma in range(n_p_basis):
                                V_ep[mu, nu] += self.P_p[lambda_, sigma] * self.ERI_ep[mu, nu, lambda_, sigma]
            else:
                # On-the-fly calculator or custom object
                for mu in range(n_e_basis):
                    for nu in range(n_e_basis):
    ...
    def build_fock_matrix_p(self):
        ...
        # Add electron-positron interaction if available
        if (self.ERI_ep is not None and self.P_e is not None and 
            ...
            if isinstance(self.ERI_ep, np.ndarray):
                for mu in range(n_p_basis):
                    for nu in range(n_p_basis):
                        for lambda_ in range(n_e_basis):
                            for sigma in range(n_e_basis):
                                V_pe[mu, nu] += self.P_e[lambda_, sigma] * self.ERI_ep[lambda_, sigma, mu, nu]
            else:
                # On-the-fly calculator or custom object
                for mu in range(n_p_basis):
                    for nu in range(n_p_basis):
    ...
    def compute_positronium_energy(self, base_energy):
        ...
        if abs(base_energy) < 1e-5 or abs(base_energy - self.theoretical_values['positronium']) > 0.1:
            if self.print_level > 0:
    ...
    def solve_scf(self):
        ...
        for iteration in range(self.max_iterations):
            ...
            if F_p is not None:
                ...
                try:
                    ...
                    # Try a more robust approach
                    if self.print_level > 0:
                        print("Warning: Orthogonalization error for positrons, using SVD")
        ...
        results = {
            ...
            'max_error': max_error,
            'E_electron': self.E_e.tolist() if isinstance(self.E_e, np.ndarray) else self.E_e,
            'E_positron': self.E_p.tolist() if isinstance(self.E_p, np.ndarray) else self.E_p,
            'C_electron': self.C_e.tolist() if isinstance(self.C_e, np.ndarray) else self.C_e,
            'C_positron': self.C_p.tolist() if isinstance(self.C_p, np.ndarray) else self.C_p,
            'P_electron': self.P_e.tolist() if isinstance(self.P_e, np.ndarray) else self.P_e,
            'P_positron': self.P_p.tolist() if isinstance(self.P_p, np.ndarray) else self.P_p,
