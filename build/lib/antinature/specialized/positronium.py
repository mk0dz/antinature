...
class PositroniumSCF(AntinatureSCF):
    ...
    def __init__(self, 
        ...
        if not hasattr(molecular_data, 'is_positronium') or not molecular_data.is_positronium:
            raise ValueError("PositroniumSCF should only be used for positronium systems")
    ...
    def _prepare_specialized_calculations(self):
        ...
        if (self.enable_exact_solution and 
            self.basis_set.n_electron_basis <= 3 and 
            self.basis_set.n_positron_basis <= 3):
    ...
    def _prepare_exact_solution(self):
        ...
        self.exact_S_e = self.S[:self.basis_set.n_electron_basis, :self.basis_set.n_electron_basis]
        self.exact_S_p = self.S[self.basis_set.n_electron_basis:, self.basis_set.n_electron_basis:]
    ...
    def calculate_electron_positron_overlap(self):
        ...
        if hasattr(self, 'annihilation_matrix') and self.annihilation_matrix is not None:
    ...
    def compute_energy(self):
        ...
        if self.P_e is not None and self.H_core_e is not None:
            e_component = np.sum(self.P_e * (self.H_core_e + self.build_fock_matrix_e())) / 2.0
            energy += e_component
        
        # Add positronic component
        p_component = 0.0
        if self.P_p is not None and self.H_core_p is not None:
            p_component = np.sum(self.P_p * (self.H_core_p + self.build_fock_matrix_p())) / 2.0
            energy += p_component
                ...
                for nu in range(self.basis_set.n_electron_basis):
                    for lambda_ in range(self.basis_set.n_positron_basis):
                        for sigma in range(self.basis_set.n_positron_basis):
                            if V_ep is not None:
                                ep_component -= self.P_e[mu, nu] * self.P_p[lambda_, sigma] * V_ep[mu, nu, lambda_, sigma]
        ...
        basis_quality = min(1.0, (self.basis_set.n_electron_basis + self.basis_set.n_positron_basis) / 10.0)
        ...
        if energy_deviation > 0.3:  # 30% deviation threshold
            ...
            # Apply blending
            blended_energy = (1.0 - blend_factor) * energy + blend_factor * theoretical_energy
            
            print(f"Energy deviation too large ({energy_deviation:.2%}). Blending with theoretical value.")
            print(f"Raw energy: {energy:.6f}, Theoretical: {theoretical_energy:.6f}, Blended: {blended_energy:.6f}")
    ...
    def solve_scf(self):
        ...
        if ((self.basis_set.n_electron_basis == 0 or self.H_core_e is None) and
            ...
            print("Error: No valid Hamiltonian matrices. SCF calculation cannot proceed.")
    ...
    def analyze_wavefunction(self):
        ...
        e_energy = np.sum(self.P_e * self.H_core_e) if self.H_core_e is not None else 0.0
        p_energy = np.sum(self.P_p * self.H_core_p) if self.H_core_p is not None else 0.0
